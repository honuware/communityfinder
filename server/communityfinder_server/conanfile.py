...
# The dependency set is a superset of honuware's own conanfile (so honuware's
# component/link declarations, pulled in via FetchContent, resolve
...
libraries = [
    ...
    # honuware Phase 11.1: libjpeg gained a CMakeInfo so ${JPEG_LIB} exists. It was
    # always a direct requirement, but with no CMake variable image_resize reached
    # libjpeg only TRANSITIVELY through libtiff. honuware now links ${JPEG_LIB}
    # explicitly, and in consumed mode that variable resolves from THIS recipe's
    # ConanLibImports.cmake -- so it must be defined here too, not only upstream.
    ...
    # case-SENSITIVE on Linux, so find_package(openssl) fails there while working on
...
class CommunityFinderRecipe(ConanFile):
    ...
    def init(self):
        # Emit the find_package calls into ConanLibImports.cmake, along with the
        # ${..._LIB} variables the CMakeLists use.
